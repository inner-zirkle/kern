...
impl Server {
	...
	pub(crate) fn tool_sign(&self, args: &serde_json::Value) -> serde_json::Value {
		...
		tool_result_json(&serde_json::json!({
			"signature": crate::base::util::hex::encode(&sig),
			"pubkey": crate::base::util::hex::encode(identity.pubkey()),
			"peer_id": crate::base::util::hex::encode(identity.peer_id()),
		}))
	}
	...
	pub(crate) fn tool_contract_grant(&self, args: &serde_json::Value) -> serde_json::Value {
		...
		tool_result_json(&serde_json::json!({
			"old_id": crate::base::util::hex::encode(old_id),
			"new_id": crate::base::util::hex::encode(new_id),
			"tombstone_sig": crate::base::util::hex::encode(&tombstone_sig),
			"amended_contract": {
				"kind": cfg.kind,
				"owners": cfg.owners,
				"writers": "allowlist",
...
mod tests {
	...
	async fn sign_returns_a_signature_the_daemons_own_pubkey_verifies() {
		...
		let digest_hex = crate::base::util::content_hash("payload to sign");
	...
	async fn contract_grant_amends_the_allowlist_and_moves_the_contract_id() {
		...
		let own_pubkey = body(&s.tool_sign(&serde_json::json!({
			"payload_hash": crate::base::util::content_hash("x")
		})))["pubkey"]
			.as_str()
			.unwrap()
			.to_string();
		...
		let grantee_hex = crate::base::util::hex::encode(grantee.pubkey());
	...
	async fn contract_grant_refuses_a_non_owner_daemon() {
		...
		let contract = serde_json::json!({
			"kind": "signed-crdt-v0",
			"owners": [crate::base::util::hex::encode(stranger.pubkey())],
			"writers": "owners-only",
		});
		let out = s.tool_contract_grant(&serde_json::json!({
			"contract": contract,
			"pubkey": crate::base::util::hex::encode(PeerIdentity::from_bytes([8u8; 32]).pubkey()),
		}));
