	...
	fn scoped(name: &str) -> Self {
		#[cfg(unix)]
		{
			...
			let path = std::env::var_os("XDG_RUNTIME_DIR")
				...
				// A deep XDG_RUNTIME_DIR (CI tmpdirs, nested worktrees) would exceed
				// SUN_LEN and kill the bind with "path must be shorter than SUN_LEN".
				// Fall back to the /tmp socket; daemon and clients share the env, so
				// both sides resolve the same endpoint.
